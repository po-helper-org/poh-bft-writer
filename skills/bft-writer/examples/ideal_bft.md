...
<Только таблица. Описание продукта, платформ и браузеров абзацем перед ней не выводится — платформенные ограничения идут строкой ИТ, неизвестное → «Вводные» (ЗМ-019). ИТ описывает интерфейс приёмки: что видит и может сделать актор. Не API-контракт (эндпоинты, протоколы, схемы запросов — зона СА).>
