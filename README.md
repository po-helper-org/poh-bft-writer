...
JIRA и Confluence — не уходя со страницы) и доску семи стадий (`To Do` →
`NEED-CUSTDEV` → `FAST-DONE` → `DEEP-REVIEW` → `DEEP-DONE` → `OKR-ADDED`,
`BFT-CANCELED`), с которой готовый БФТ одной кнопкой «Добавить в OKR» уходит в
квартальное планирование — квартал, спринты по фазам, команды и ответственные
пишутся в задачу Backlog.md в формате карточки KR раздела «Управление целями».
...
## Опционально: раскадровка интерфейса (`bft-wireframing`)

`/bft-wireframe` превращает уже написанные сценарии `How to demo` в шапке документа
в StepByStep-визуализацию: экранные wireframe-кадры (реальные виджеты — radio, toggle,
таблицы, а не прямоугольники с текстом) в рамке браузера или телефона, кадры соединены
стрелками, под каждым — подпись. Все кадры одного документа делят одну навигацию —
различается только подсвеченный активный пункт, так раскадровка читается как один продукт.

**Не входит в контур по умолчанию.** Ни `/bft-fast`, ни `/bft-deep` эту команду не вызывают —
только явный запуск `/bft-wireframe <epic_slug>`. Не источник требований: не придумывает,
что делает система, и не меняет ни слова уже согласованного сценария — только рисует то,
что там написано. Не занимает высоту БФТ: блок раскадровки живёт в шапке рядом с
`How to demo`, не в каноне, и не подменяет ни «Требования к интерфейсам\*», ни
«План демонстрации».

Рендерит блоки `bft-html-export.py` при сборке страницы ревью — сам `.md` несёт
компактный DSL-текст (десятки строк на сценарий), не готовую картинку. Требует
разового `npm install` в `skills/bft-wireframing/scripts/wireloom-runtime/`
(рендер — библиотека [Wireloom](https://github.com/StardockCorp/Wireloom), MIT,
0 runtime-зависимостей); без него страница честно показывает
`[УТОЧНИТЬ: навык bft-wireframing не установлен]` вместо картинки — сборка документа
от этого не падает.
