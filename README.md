...
`workspaceRoot` (по строке `id: bft-requirements`). Подробности, переменные
окружения и таблица «артефакты → стадия» — в [`plugin/README.md`](plugin/README.md).

> **Грабля.** Штатная `dsh plugin add` может отрапортовать успех, не изменив
> манифест профиля; тогда пакет и строку бандла придётся дописать в профиль
> руками. После установки или обновления плагина харнесс нужно перезапустить —
> на живую он плагин не подхватывает.

> **Грабля.** Слой профиля заменяет конфигурацию пакета целиком. Задав в нём
> `workspaceRoot`, вы теряете всё, что объявлял `cordis.patch.yml` пакета;
> умолчания поэтому живут в коде, а посмотреть, что раздел получит на самом
> деле, можно через `dsh --profile web --dump-config`.
