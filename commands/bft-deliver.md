...
**Инструмент:** `python3 skills/bft-writer/scripts/bft-confluence-macros.py <epic>.md --confluence-map <map.json> --format storage|wiki -o <converted>.txt`. `<map.json>` — `{"<pageId>": {"title": "...", "space": "SPACE"}}`, собери сам через `confluence_get_page` для каждого pageId из Этапа 1.5 (скрипт Confluence не читает, только подставляет уже известные title/space). Jira-ссылкам map не нужен — ключ уже в URL. Прогони на всём теле документа перед публикацией — это и есть шаг «конвертируй все упоминания» ниже, а не ручная правка построчно. pageId без записи в map (или без `title` в записи) скрипт не трогает — соответствующая ссылка остаётся исходной markdown-ссылкой (в любом `--format`, storage или wiki), не превращается в битый/наполовину-макрос; предупреждение об этом идёт в stderr — не тратить время на map для ссылок, которые не обязательно превращать в макрос.
