...
| 3 | [Детальная страница](03-detail-page.md) | Документ требования, чат с Claude Code: правки, вопросы, отгрузка в JIRA и Confluence |
| 4 | [Доска по стадиям](04-board.md) | Семь колонок пайплайна, переходы, «Добавить», «Добавить в OKR» |
...
- Чат на детальной странице — Claude Code CLI (`claude` на машине харнесса, ключ
  `claudeBin` профиля); для отгрузки `/bft-deliver` из чата — MCP-сервер Jira/Confluence,
  разрешённый CLI через `claudeArgs`, и `tracker_projects`, `wiki_space`,
  `bft_parent_page_id` в корневом `bft-config.md`.

## Чего раздел не делает

- **Не отправляет ничего в чат харнесса сам.** «Работать в чате» в превью и «Обновить» кладут
  черновик в композер; Enter жмёшь ты. Исключение — чат на детальной странице: он идёт через
  Claude Code CLI, и «В чат» отправляет сразу (сценарий 3).
